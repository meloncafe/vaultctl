...
def _get_docker_compose_command() -> list[str]:
    ...
    # 3. 둘 다 없으면 오류
    console.print("[red]✗[/red] docker compose가 설치되지 않았습니다.")
    raise typer.Exit(1)
...
def list_docker():
    ...
    try:
        items = _get_auth_client().kv_list(settings.kv_mount, settings.kv_docker_path)
    ...
    for item in sorted(items):
        name = item.rstrip("/")
        try:
            data = _get_auth_client().kv_get(settings.kv_mount, _get_docker_path(name))
...
def put_docker(
    ...
    if merge:
        try:
            existing = _get_auth_client().kv_get(settings.kv_mount, _get_docker_path(name))
    ...
    try:
        _get_auth_client().kv_put(settings.kv_mount, _get_docker_path(name), new_data)
...
def delete_docker(
    ...
    try:
        _get_auth_client().kv_delete(settings.kv_mount, _get_docker_path(name))

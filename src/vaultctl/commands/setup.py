...
def _save_apt_config(config: dict) -> None:
    """Save APT config using template / 템플릿을 사용하여 APT 설정 저장."""
    APT_BASE.mkdir(parents=True, exist_ok=True)
    
    content = templates.render_apt_config(config)
...
def _setup_apt_gpg(config: dict) -> None:
    """Setup GPG key for APT signing using template / 템플릿을 사용하여 APT 서명용 GPG 키 설정."""
